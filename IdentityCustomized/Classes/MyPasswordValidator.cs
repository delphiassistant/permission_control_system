...
using IdentityCustomized.Resources;
using Microsoft.AspNet.Identity;

namespace Barnamenevis.AspNet.Identity
{
    /// <summary>
    ...
    public class MyPasswordValidator : IIdentityValidator<string>
    {
        ...
        public virtual Task<IdentityResult> ValidateAsync(string item)
        {
            ...
            {
                errors.Add(String.Format(CultureInfo.CurrentCulture, IdentityMessages.PasswordTooShort, RequiredLength));
            }
            if (RequireNonLetterOrDigit && item.All(IsLetterOrDigit))
            {
                errors.Add(IdentityMessages.PasswordRequireNonLetterOrDigit);
            }
            if (RequireDigit && item.All(c => !IsDigit(c)))
            {
                errors.Add(IdentityMessages.PasswordRequireDigit);
            }
            if (RequireLowercase && item.All(c => !IsLower(c)))
            {
                errors.Add(IdentityMessages.PasswordRequireLower);
            }
            if (RequireUppercase && item.All(c => !IsUpper(c)))
            {
                errors.Add(IdentityMessages.PasswordRequireUpper);
            }
            if (errors.Count == 0)
            {
